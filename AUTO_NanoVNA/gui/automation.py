...
def parse_frequency_value(value: str) -> float:
    ...
    if not match:
        raise ValueError("Use a value with a unit: Hz, kHz, MHz or GHz.")
    number = float(match.group(1).replace(",", "."))
    return number * FREQUENCY_UNITS[match.group(2).lower()]


def parse_python_control_line(line: str) -> tuple[str, Any] | None:
    value = line.strip()
    ...
    if range_match:
        ...
        return f"range_{side}", parse_frequency_value(range_match.group(2))
...
class AutomationWindow(tk.Toplevel):
    ...
    def __init__(
        ...
        self.description_vars = {
            "Tytuł badania": tk.StringVar(value="Automatyczna seria NanoVNA"),
            "Operator": tk.StringVar(),
            "Badany obiekt": tk.StringVar(),
            "Temperatura": tk.StringVar(),
            "Wilgotność": tk.StringVar(),
            "Długość linii": tk.StringVar(),
        }
        self._build()
...
def send_gcode(ser, command):
    ...
    while True:
        ...
        if response_lower.startswith("ok"):
            return
        if response_lower.startswith("error") or response_lower.startswith("!!"):
...
def main():
    ...
    try:
        ...
        for position, x in enumerate(POSITIONS_X, start=1):
            ...
            print(
                f"nanoOK_{current_number}|name={filename}",
                flush=True,
            )
            current_number += 1
        ...
        move(ser, "G28 X Y")
        print(
            f"Finished. Saved {current_number - 1} measurements.",
            flush=True,
        )

    except serial.SerialException as error:
        print(f"Serial-port error: {error}", flush=True)
    except RuntimeError as error:
        print(f"Automation error: {error}", flush=True)
    finally:
...
'''
    ...
    def _build(self) -> None:
        ...
        formats_frame.grid(row=2, column=0, columnspan=5, sticky="w", pady=(8, 0))
        ...
        ttk.Label(data_section, text=self._t("s2p_assumption", ".s2p używa założenia S12=S21 i S22=S11."), wraplength=900).grid(row=3, column=0, columnspan=5, sticky="w", padx=6, pady=(4, 0))
        ...
        ttk.Label(python_header, text=self._t("python_instruction", "Komendy muszą być wysłane przez print(..., flush=True). nanoOK_<index>|name=<name> zapisuje pomiar. nanoDataMem ustawia Data → Mem. Zakres: nanoRangeStart:10MHz i nanoRangeStop:100MHz. Jednorazowy zapis: nanoCSV-s11-s21-zs11-memory, nanoS1P-s11 lub nanoS2P-s11-s21. Pola: s11, s21, s11ri, s21ri, zs11, vswr, memory, markers, tdr, all. Nie używaj myślników w nazwach; myślnik rozdziela elementy komendy."), wraplength=840, justify="left").pack(side="left", fill="x", expand=True)
        ...
        self.python_log = tk.Text(self.python_frame, height=6, wrap="word", font=("Consolas", 8), state="disabled")
        ...
        ttk.Combobox(self.threshold_frame, textvariable=self.metric_var, values=self.METRICS, state="readonly", width=28).grid(row=0, column=1, padx=4)
        ...
        display_map = {
            "Tytuł badania": self._t("study_title", "Tytuł badania"),
            "Operator": self._t("operator", "Operator"),
            "Badany obiekt": self._t("object", "Badany obiekt"),
            "Temperatura": self._t("temperature", "Temperatura"),
            "Wilgotność": self._t("humidity", "Wilgotność"),
            "Długość linii": self._t("line_length_short", "Długość linii"),
        }
    ...
    def _load_reference_file(self) -> None:
        filename = filedialog.askopenfilename(parent=self, title=self._t("load_reference", "Wczytaj plik referencyjny"), filetypes=[("Measurement files", "*.csv *.s1p *.s2p"), ("All files", "*.*")])
        if not filename:
            return
        try:
    ...
    def _add_images(self) -> None:
        files = filedialog.askopenfilenames(parent=self, title=self._t("choose_images", "Wybierz zdjęcia"), filetypes=[(self._t("images", "Obrazy"), "*.png *.jpg *.jpeg *.webp *.bmp"), (self._t("all_files", "Wszystkie pliki"), "*.*")])
        for filename in files:
            path = Path(filename)
            if path not in self.image_paths:
                self.image_paths.append(path)
        self.images_var.set(f"Wybrano: {len(self.image_paths)}")
    ...
    def _start_python(self) -> None:
        ...
        if not self.python_command:
            self.active = False
            messagebox.showerror(self._t("python_missing_title", "Brak Pythona"), self._t("python_interpreter_not_found", "Nie znaleziono interpretera Python."), parent=self)
            return
        try:
            self.process = subprocess.Popen([*self.python_command, "-u", str(script)], stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True, cwd=str(Path(self.folder_var.get()).expanduser()), env=environment, bufsize=1, **process_options)
        except Exception as exception:
            self.active = False
            messagebox.showerror(self._t("python_code_error", "Błąd kodu Python"), str(exception), parent=self)
            return
    ...
    def _python_line(self, line: str) -> None:
        self.python_log.configure(state="normal")
        self.python_log.insert("end", line + "\n")
        self.python_log.see("end")
        self.python_log.configure(state="disabled")
        if not self.active:
            return
        ...
        if event:
            ...
            if self.range_change_active:
                ...
                self.pending_fields_override = None
                self.pending_formats_override = None
                self.status_callback(f"nanoOK_{index} is waiting for the sweep-range change to finish.")
                return
            ...
            return
        try:
            control = parse_python_control_line(line)
        except Exception as exception:
            self.status_callback(f"Python command error: {exception}")
            return
        if control is None:
            return
        ...
        if command == "data_mem":
            if self.range_change_active:
                ...
                self.status_callback("nanoDataMem oczekuje na nowy pomiar po zmianie zakresu.")
            else:
    ...
    def _apply_data_mem(self) -> None:
        try:
            ...
            self.status_callback(f"nanoDataMem error: {exception}")
    ...
    def _apply_format_override(self, format_name: str, tokens: list[str]) -> None:
        ...
        if format_name == "s1p":
            ...
            if invalid:
                self.status_callback("nanoS1P obsługuje tylko pole s11.")
            return
        if format_name == "s2p":
            invalid = [token for token in tokens if token not in {"s11", "s21"}]
            if invalid:
                self.status_callback("nanoS2P obsługuje pola s11 i s21.")
            return
        if not tokens:
            return
        ...
        for token in tokens:
            ...
            if field_name is None:
                self.status_callback(f"Nieznane pole nanoCSV: {token}")
                continue
            self.pending_fields_override[field_name] = True
        self.status_callback("Ustawiono jednorazowy format następnego nanoOK.")
    ...
    def _try_apply_range(self) -> None:
        ...
        if start_hz <= 0 or stop_hz <= start_hz:
            self.status_callback("Błędny zakres: stop musi być większy od startu.")
            return
        if self.range_change_active:
            self.status_callback("Zmiana zakresu już trwa.")
            return
        self.range_change_active = True
        self.status_callback(f"Ustawianie zakresu {start_hz:g}–{stop_hz:g} Hz…")
        try:
    ...
    def _range_ready(self) -> None:
        ...
        self.status_callback("Zakres ustawiony i wykonano pierwszy pomiar.")

    def _range_failed(self, exception: Exception) -> None:
        self.range_change_active = False
        self.range_data_mem_pending = False
        self.queued_python_events.clear()
        self.status_callback(f"Sweep-range change requested by Python failed: {exception}")
    ...
    def on_measurement(self, metrics: dict[str, float]) -> None:
        ...
        if self._deadline_expired():
            self._deadline_callback()
            return
        value = metrics.get(self.metric_var.get())
    ...
    def _trigger(
        ...
        if not any(formats.values()):
            self.status_callback(f"Automatic save {index} failed: no output format selected.")
            return False
        ...
        try:
            ...
            self.status_callback(f"Automatic save {index} failed: {exception}")
            return False
        self.saved_indices.add(index)
        self.saved_files.extend(paths)
        if mode == "python":
            self.state_var.set(f"{self._t('series_active', 'Seria aktywna')}: {len(self.saved_indices)} | ostatni indeks: {index}")
        else:
            count = int(self.count_var.get())
            self.state_var.set(f"{self._t('series_active', 'Seria aktywna')}: {len(self.saved_indices)}/{count} | ostatni indeks: {index}")
    ...
    def _finish(self, reason: str = "count") -> None:
        ...
        if reason.startswith("process_error_"):
            state += f" | kod procesu {reason.rsplit('_', 1)[-1]}"
        self.state_var.set(state)
        if was_active and self.generate_report_var.get():
            description = {key: variable.get() for key, variable in self.description_vars.items()}
            description[self._t("notes", "Opis / uwagi")] = self.notes_text.get("1.0", "end-1c").strip()
            description[self._t("series_end_reason", "Powód zakończenia serii")] = reason
            report_path = Path(self.folder_var.get()).expanduser() / f"raport_serii_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"
            try:
    ...
    def _on_close(self) -> None:
        if self.active and not messagebox.askyesno("Zamknąć?", "Seria jest aktywna. Zatrzymać ją i zamknąć okno?", parent=self):
            return
        self.closing = True
        self.stop()
        self.destroy()
