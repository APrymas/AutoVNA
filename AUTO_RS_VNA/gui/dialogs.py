...
POINT_PRESETS = (2, 11, 21, 51, 101, 201, 401, 801, 1001, 1601, 2001, 3201, 4001)


class RangeDialog(tk.Toplevel):
    def __init__(self, parent, current: tuple[float, float, int], callback, tr=None):
        super().__init__(parent)
        self.translate = lambda key, fallback=None: tr(key, fallback) if tr else (fallback if fallback is not None else key)
        ...
        self.points_var = tk.StringVar(value=str(points if points in POINT_PRESETS else 201))
        ...
        ttk.Combobox(frame, textvariable=self.unit_var, values=["Hz", "kHz", "MHz", "GHz"], state="readonly", width=7).grid(row=0, column=3)
        ttk.Label(frame, text=translate("points", "Punkty")).grid(row=2, column=0, sticky="w", pady=4)
        ttk.Combobox(frame, textvariable=self.points_var, values=[str(value) for value in POINT_PRESETS], state="readonly", width=16).grid(row=2, column=1, padx=5)
        ttk.Label(frame, text=translate("points_range_note", "Dozwolony zakres ZVL: 2–4001 punktów."), wraplength=270).grid(row=2, column=2, columnspan=2, sticky="w", padx=(8, 0))
    ...
    def accept(self):
        try:
            multiplier = {"Hz": 1.0, "kHz": 1e3, "MHz": 1e6, "GHz": 1e9}[self.unit_var.get()]
            ...
            if not 5_000 <= start < stop <= 15_000_000_000 or not 2 <= points <= 4001:
                raise ValueError
        except Exception:
            messagebox.showerror(self.translate("error", "Błąd"), self.translate("range_error_message", "Zakres ZVL13: 5 kHz–15 GHz; liczba punktów: 2–4001."), parent=self)
            return
        self.destroy()
...
class SaveSettingsDialog(tk.Toplevel):
    FIELD_LABELS = {
        ...
        "s12_db_phase": "S12: dB i faza",
        "s22_db_phase": "S22: dB i faza",
        "s11_complex": "S11: Re i Im",
        "s21_complex": "S21: Re i Im",
        "s12_complex": "S12: Re i Im",
        "s22_complex": "S22: Re i Im",
    ...
    FORMAT_LABELS = {"csv": "CSV AutoRS VNA", "s1p": "Touchstone .s1p", "s2p": "Touchstone .s2p"}
    ...
    def __init__(self, parent, fields: dict[str, bool], formats: dict[str, bool], callback, tr=None):
        ...
        ttk.Label(formats_frame, text=translate("s2p_real_data", ".s2p zapisuje rzeczywiście zmierzone S11, S21, S12 i S22."), wraplength=510).grid(row=1, column=0, columnspan=3, sticky="w", padx=8, pady=(5, 0))
    ...
    def accept(self):
        ...
        if not any(formats.values()):
            messagebox.showerror(self.translate("error", "Błąd"), self.translate("save_format_required", "Wybierz co najmniej jeden format pliku."), parent=self)
            return
...
class TDRSettingsDialog(tk.Toplevel):
    def __init__(self, parent, values: dict, callback, current_sweep: tuple[float, float] | None = None, tr=None):
        ...
        row += 1
        for label_key, label, variable in (
            ("tdr_vf", "VF — współczynnik prędkości", self.vf_var),
            ("tdr_z0", "Z0 — impedancja odniesienia [Ω]", self.z0_var),
        ):
            ttk.Label(frame, text=translate(label_key, label)).grid(row=row, column=0, sticky="w", pady=4)
            ttk.Entry(frame, textvariable=variable, width=20).grid(row=row, column=1, sticky="w", padx=6)
            row += 1
        ...
        row += 1
        for label_key, label, variable in (
            ("tdr_max_distance", "Maks. odległość wykresu [m], 0 = auto", self.max_distance_var),
            ("tdr_zero_offset", "Przesunięcie zera odległości [m]", self.zero_offset_var),
        ):
            ttk.Label(frame, text=translate(label_key, label)).grid(row=row, column=0, sticky="w", pady=4)
            ttk.Entry(frame, textvariable=variable, width=20).grid(row=row, column=1, sticky="w", padx=6)
            row += 1
        ...
        if current_sweep and current_sweep[1] > current_sweep[0]:
            ...
            try:
                resolution = 299_792_458.0 * float(self.vf_var.get().replace(",", ".")) / (2.0 * (stop - start))
    ...
    def accept(self):
        ...
        try:
            ...
            return
        values = {
            "axis_mode": self.axis_var.get(), "velocity_factor": velocity_factor,
            "z0_ohm": reference_impedance, "window": self.window_var.get(),
            "max_distance_m": max_distance, "zero_offset_m": zero_offset,
...
class ModelOptionsDialog(tk.Toplevel):
    ...
    def __init__(self, parent, callback, tr=None):
        ...
        self.translate = lambda key, fallback=None: tr(key, fallback) if tr else (fallback if fallback is not None else key)
        translate = self.translate
        self.title(translate("model_title", "Dopasowanie modelu linii"))
        self.resizable(False, False)
        self.transient(parent)
        self.grab_set()
        self.callback = callback
        ...
        ttk.Label(
            ...
            text=translate(
                "model_time_warning",
                "Na czas obliczeń zostanie wstrzymane tylko cykliczne pobieranie danych przez GUI. ZVL może nadal wykonywać ciągły sweep.",
            ),
            wraplength=500,
    ...
    def accept(self):
        ...
        if length.lower() != "unknown":
            try:
                ...
                messagebox.showerror(self.translate("error", "Błąd"), self.translate("line_length_invalid", "Długość musi być dodatnia albo 'unknown'."), parent=self)
                return
...
class CalibrationModeDialog(tk.Toplevel):
    

    MODES = (
        ("p1", "cal_p1_title", "cal_p1_description", "P1 – pełna 1-portowa", "Kalibracja portu 1 dla pomiaru S11: OPEN1, SHORT1 i MATCH1."),
        ("p2", "cal_p2_title", "cal_p2_description", "P2 – pełna 1-portowa", "Kalibracja portu 2 dla pomiaru S22: OPEN2, SHORT2 i MATCH2."),
        ("forward", "cal_forward_title", "cal_forward_description", "P1 → P2 – 1-path 2-port", "Kalibracja toru forward dla S11 i S21: OPEN1, SHORT1, MATCH1 oraz THRU 1→2."),
        ("tosm", "cal_tosm_title", "cal_tosm_description", "P1 ↔ P2 – pełna TOSM 2-portowa", "Pełna kalibracja dla S11, S21, S12 i S22: OSM na obu portach oraz THRU."),
    )

    def __init__(self, parent, callback, tr=None, palette=None):
        super().__init__(parent)
        translate = lambda key, fallback: tr(key, fallback) if tr else fallback
        self.title(translate("calibration_window_title", "Kalibracja ZVL13"))
        self.resizable(False, False)
        self.transient(parent)
        self.grab_set()
        self.callback = callback
        ...
        if palette:
            self.configure(bg=palette.get("bg", "#171a1f"))
        ...
        ttk.Label(
            frame,
            text=translate(
                "calibration_choose_mode",
                "Wybierz procedurę. W czasie kalibracji sweep ciągły może zostać chwilowo zatrzymany, a po zakończeniu zostanie przywrócony.",
            ),
            wraplength=610,
            justify="left",
        ).grid(row=0, column=0, sticky="w", pady=(0, 10))

        for row, (token, title_key, description_key, title_fallback, description_fallback) in enumerate(self.MODES, start=1):
            title = translate(title_key, title_fallback)
            description = translate(description_key, description_fallback)
            option = ttk.Frame(frame)
            option.grid(row=row, column=0, sticky="ew", pady=4)
            ttk.Radiobutton(
                option,
                text=title,
                value=token,
            ...
            ttk.Label(option, text=description, style="Info.TLabel", wraplength=590).pack(
                anchor="w", padx=(25, 0), pady=(1, 0)
            )

        buttons = ttk.Frame(frame)
        buttons.grid(row=len(self.MODES) + 1, column=0, sticky="e", pady=(14, 0))
        ttk.Button(buttons, text=translate("cancel", "Anuluj"), command=self.destroy).pack(side="right", padx=4)
        ttk.Button(buttons, text=translate("start", "Rozpocznij"), command=self.accept).pack(side="right", padx=4)
...
class ProcessLogWindow(tk.Toplevel):
    ...
    def __init__(self, parent, title: str, stop_callback=None, tr=None, palette=None):
        super().__init__(parent)
        self.title(title)
        translate = lambda key, fallback: tr(key, fallback) if tr else fallback
        ...
        self.text = tk.Text(
            ...
            wrap="word",
            font=("Consolas", 10),
            bg=palette.get("panel", "#222730"),
            fg=palette.get("fg", "#f5f7fa"),
            insertbackground=palette.get("fg", "#f5f7fa"),
            selectbackground=palette.get("accent", "#4da3ff"),
